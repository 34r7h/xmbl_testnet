...
- **Single source of truth:** [`xn/src/config.js`](../xn/src/config.js) —
  `NODE_CONFIG_SCHEMA`, `defaultConfig()`, `validateConfig()`, `loadConfig()`.
- **Canonical example:** [`xn/config.example.json`](../xn/config.example.json).
- The example and this document are checked against the schema by
  `xn/__tests__/config.test.js`, so all three stay in lock-step.
...
> component. The node-daemon config lives at `xn/config.example.json`.
